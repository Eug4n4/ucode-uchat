...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        int bytes_received = recv(server_fd, buf, BUF_SIZE, 0);
        if (bytes_received == 0) {
        ...
        // Exit the loop if response type indicates completion
        if (response_type->valueint == 1 || response_type->valueint == 3) {
            break;
        }
        // Continue communication with a new private chat request after main loop
    ...
    bytes_received = recv(server_fd, buf, BUF_SIZE, 0);
    if (bytes_received == 0) {
        printf("Server closed connection\n");
        exit(-1);
    } else if (bytes_received == -1) {
    ...
    close(server_fd); // Close the connection when done
    return 0;
    }
