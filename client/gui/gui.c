...
void sign_in_connect_signals(t_gtk_sign_in *gtk_sign_in) {
    GtkButton *btn_sign_in = GTK_BUTTON(gtk_builder_get_object(gtk_sign_in->builder, "btn_sign_in"));
    GtkButton *btn_sign_up = GTK_BUTTON(gtk_builder_get_object(gtk_sign_in->builder, "btn_sign_up"));

    g_signal_connect(gtk_sign_in->window, "destroy", G_CALLBACK(gtk_main_quit), NULL);
    g_signal_connect(btn_sign_in, "clicked", G_CALLBACK(on_login_clicked), gtk_sign_in);
    // Add other button connections as needed
}
